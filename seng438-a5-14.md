...
To determine the MTTFmin for the SUT, we used trial and error method using the RDC, and found that with MTTF of 3.75 minutes, the failure plot stayed outside of the reject region of the chart. When the MTTF was halved by increasing the "per number of input events" value, the SUT data plot was shifted to the left, while it shifted to the right when the same value was decreased and MTTF value was doubled.

# Comparison of Results

By utilizing the minimum MTTF, we can make a comparison between the results obtained from the RDC and the C-SFRAT. For the C-SFRAT, the minimum MTTF failure rate required for acceptance was determined to be 0.2857. On the other hand, the RDC plot indicated a minimum acceptance value of 0.4 for MTTF.
